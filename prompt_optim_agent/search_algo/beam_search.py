...
class BeamSearch(SearchAlgo):
    ...
    def test_and_log_node(self, node:BeamNode, eval=False, eval_type='test'):
        ...
        if node.parent is not None:
            self.logger.info(f'node {node.id}:    parent: {node.parent.id} | depth: {node.depth} | eval: {node.eval_metric} | test: {node.test_metric} | NMI: {node.nmi} | AMI: {node.ami} | ARI: {node.ari} | ACC: {node.acc}\nprompt: {node.prompt}')
        else:
            self.logger.info(f'node {node.id}:    parent: N/A | depth: {node.depth} | eval: {node.eval_metric} | test: {node.test_metric} | NMI: {node.nmi} | AMI: {node.ami} | ARI: {node.ari} | ACC: {node.acc}\nprompt: {node.prompt}')
        self.logger.info(f'---------------------')
